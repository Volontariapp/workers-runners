...
@Injectable()
export class FallbackDeleteBadgeHandler implements IJobHandler<
  typeof JobMessagingType.FALLBACK_DELETE_BADGE
> {
  private readonly logger = new Logger({
    context: FallbackDeleteBadgeHandler.name,
  });

  public readonly jobType = JobMessagingType.FALLBACK_DELETE_BADGE;

  async handle(
    job: JobOf<typeof JobMessagingType.FALLBACK_DELETE_BADGE>,
  ): Promise<void> {
    this.logger.info(
