...
@Injectable()
export class FallbackGetPostLikersHandler implements IJobHandler<
  typeof JobMessagingType.FALLBACK_GET_POST_LIKERS
> {
  private readonly logger = new Logger({
    context: FallbackGetPostLikersHandler.name,
  });

  public readonly jobType = JobMessagingType.FALLBACK_GET_POST_LIKERS;

  async handle(
    job: JobOf<typeof JobMessagingType.FALLBACK_GET_POST_LIKERS>,
  ): Promise<void> {
    this.logger.info(
