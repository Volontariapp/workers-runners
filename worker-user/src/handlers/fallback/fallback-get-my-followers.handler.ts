...
@Injectable()
export class FallbackGetMyFollowersHandler implements IJobHandler<
  typeof JobMessagingType.FALLBACK_GET_MY_FOLLOWERS
> {
  private readonly logger = new Logger({
    context: FallbackGetMyFollowersHandler.name,
  });

  public readonly jobType = JobMessagingType.FALLBACK_GET_MY_FOLLOWERS;

  async handle(
    job: JobOf<typeof JobMessagingType.FALLBACK_GET_MY_FOLLOWERS>,
  ): Promise<void> {
    this.logger.info(
