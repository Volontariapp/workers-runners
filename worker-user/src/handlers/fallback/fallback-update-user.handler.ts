...
@Injectable()
export class FallbackUpdateUserHandler implements IJobHandler<
  typeof JobMessagingType.FALLBACK_UPDATE_USER
> {
  private readonly logger = new Logger({
    context: FallbackUpdateUserHandler.name,
  });

  public readonly jobType = JobMessagingType.FALLBACK_UPDATE_USER;

  async handle(
    job: JobOf<typeof JobMessagingType.FALLBACK_UPDATE_USER>,
  ): Promise<void> {
    this.logger.info(
