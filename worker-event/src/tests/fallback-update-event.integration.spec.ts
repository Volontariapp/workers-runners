...
import { FallbackUpdateEventFactory } from './factories/fallback-update-event.factory.js';
...
describe('FallbackUpdateEventHandler (Integration)', () => {
  let testEnv: TestEnvironment;
  let databaseCleaner: DatabaseCleaner;
  let queue: Queue;
  let eventService: EventService;
  let auditRepo: JobAuditRepository;
  let handlerSpy: jest.SpiedFunction<FallbackUpdateEventHandler['handle']>;
  ...
  beforeAll(async () => {
    ...
    const handler = testEnv.get(FallbackUpdateEventHandler);
  ...
  it('should process a FALLBACK_UPDATE_EVENT job exactly once', async () => {
    const event = await eventService.create(EventFactory.buildEventData());

    const payload = FallbackUpdateEventFactory.buildPayload(event.id);
    ...
    await queue.add(JobMessagingType.FALLBACK_UPDATE_EVENT, jobData, {
      jobId: randomUUID(),
    });
    ...
    expect(updatedEvent.name).toBe(payload.payload.event?.title);
    expect(updatedEvent.description).toBe(payload.payload.event?.description);
  });
  ...
  it('should retry job on failure and succeed on the second attempt, updating audit to COMPLETED', async () => {
    ...
    const updateSpy = jest.spyOn(EventService.prototype, 'update');
  ...
  it('should fail permanently when retries are exhausted and update audit to FAILED', async () => {
    const payload = FallbackUpdateEventFactory.buildPayload();
    const jobId = randomUUID();
    const jobData = {
      emitter: 'test',
      emitterId: randomUUID(),
