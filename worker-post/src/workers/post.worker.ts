...
import { type JobMessagingType, PostQueue } from '@volontariapp/messaging';
import { PublishPostHandler } from './handlers/publish-post.handler.js';
import type { IJobHandler } from './job-handler.interface.js';

@Injectable()
@Processor(PostQueue.POST)
export class PostWorker extends BaseWorker<JobMessagingType> {
  private readonly handlerMap: Map<JobMessagingType, IJobHandler>;

  constructor(
    @Inject(JobAuditRepository) auditRepo: JobAuditRepository,
    @Inject(PublishPostHandler) handler: PublishPostHandler,
  ) {
    super(auditRepo);
